...
int main(int argc, char *argv[])
{
    ...
    FILE *log_tempo = fopen("resultados/tempos_execucao.txt", "w");
    fprintf(log_tempo, "Execução Sequencial:\n\n");
    FILE *log_tempo_2 = fopen("resultados/tempos_execucao_2.txt", "w");
    fprintf(log_tempo_2, "Execução Sequencial:\n\n");
    ...
    for (int argv_idx = 2; argv_idx < argc; argv_idx += 2)
    {
        ...
        fprintf(log_tempo, "%s: %.4f segundos\n", nome_arquivo_resultado, tempo_execucao);
        fprintf(log_tempo_2, "%d: %.4f\n", TEST_SIZE, tempo_execucao);
